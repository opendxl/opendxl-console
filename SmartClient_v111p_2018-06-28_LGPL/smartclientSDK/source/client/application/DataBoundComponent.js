...
fieldValuesAreEqual : function (field, value1, value2) {
    ...
    return isc.DynamicForm.compareValues(value1, value2, field, true);
},
...
exportData : function (requestProperties, callback) {
    ...
    // include implicitCriteria in the export fetch
    var criteria = isc.DS.compressNestedCriteria(
            isc.DS.combineCriteria(this.getImplicitCriteria(), this.getCriteria())
    );
...
setImplicitCriteria : function (crit, invalidate) {
    ...
    ;
    if (!isRS && this.isGrouped && this.originalData && isc.isA.Tree(d)) {
        // LG.groupTree is an isc.Tree - implicitCrit is applied to the originalData because
        // LG.groupTree is transient and its getCriteria() returns the crit from the originalData
        d = this.originalData;
        isRS = true;
    }
    if (!crit) {
...
createDataModel : function (filterCriteria, operation, context) {
    ...
    var resultSet = isc.addProperties({}, this.dataProperties);
...
refreshData : function (callback) {
        ...
        (resultSet.isMultiDSTree() || resultSet.loadDataOnDemand))
    {
        ...
        this._refreshResultTreeData(callback);
        return;
    }
    ...
    var dataCriteria = this.getCriteria(),
...
getRecordsMatchingHilite : function (hilite, data) {
    ...
    if (dataSource) {
        var undef, drop = dataSource.dropUnknownCriteria;
...
evaluateCriterion : function (record, criterion) {
    ...
    var ds = isc.Canvas.defaultFilteringDataSource;
    if (!ds) {
        ds = isc.Canvas.defaultFilteringDataSource = isc.DataSource.create({
            clientOnly: true,
            addGlobalId: false,
    ...
    if(op.valueType === "fieldName") {
        ...
        var otherFieldValue = isc.DataSource.getPathValue(record, criterionValues.value, 
                                                          otherField);
        criterionValues.otherValue = convertToAbsoluteDateOrKeepOriginalValue(otherFieldValue);
    } else if (op.valueType === "criteria") {
        // If we're dealing with a criteria operator such as AND, NOT & OR, lets add the
        // criterion to the criterionValues object.
        criterionValues.criterion = criterion;
        criterionValues.record = record;
    }

    var condition = op.condition(criterion.fieldName, fieldValue, criterionValues, ds, 
                                 isDateField);
...
getExportFieldValue : function (record, field, fieldIndex, exportValueFields) {
    ...
    return this.htmlUnescapeExportFieldValue(
        this.getStandaloneFieldValue(record, field.name, !!exportValueFields, true), stripTags);
},
...
getFormattedValue : function (record, fieldName, value, alwaysFormatValue) {
    return value;
},
...
getStandaloneFieldValue : function (record, fieldName, unformatted, alwaysFormatValue) {
    ...
    if      (field.userFormula) value = this.getFormulaFieldValue(field, record);
        ...
        if (!unformatted) value = this.getFormattedValue(record, fieldName, value, alwaysFormatValue);
    }
