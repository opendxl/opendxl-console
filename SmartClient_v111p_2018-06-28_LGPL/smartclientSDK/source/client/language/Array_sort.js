...
sortByProperties : function () {
    ...
    for (var i = 0; i < props.length; i++) {
        ...
        if (isc.isA.Function(normalizer)) {
            ...
            if (context && (context.getUnderlyingField || context.getField)) {
                var field;
                if (context.getUnderlyingField) field = context.getUnderlyingField(property);
                else if (context.getField) field = context.getField(property);
