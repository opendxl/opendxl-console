...
var libs = 
	[
        ...
        //"../../SDKPackage/webroot/aceeditor/ace.js", // aceeditor is in the webroot
        "widgets/AceEditor"
	];
...
// the following statement allows a page that is not in the standard location to take advantage
// of dynamically loaded scripts by explicitly setting the window.isomorphiDir variable itself.
...
(function () {
    function loadLib(lib, hash) {
        if (hash == null) hash = "";
        document.write("<"+"script src='" + window.isomorphicDir + "client/" + lib + ".js" +
                       hash + "' type='text/javascript' charset='UTF-8'><"+"/script>");
    }

    loadLib("language/startDefiningFramework", "#module=AceEditor");
    for (var i = 0, l = libs.length; i < l; ++i) {
        if (!libs[i]) continue;
        if (window.UNSUPPORTED_BROWSER_DETECTED == true) break;
        loadLib(libs[i]);
    }
    loadLib("language/stopDefiningFramework", "#module=AceEditor");
})();
